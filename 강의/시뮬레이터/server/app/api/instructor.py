...
tenant_id 에 '*' 를 주면 전 테넌트에 동시 주입된다(2일차 실습 Step 2 처럼
강사가 한 번에 쏴 주는 상황).
"""
...
def _warnings() -> list[dict]:
    ...
    3일차 는 제어를 개방한 상태로 진행하는데, 이때 배속이 1이면
...
async def time_scale(
    ...
    그러면 2일차의 "드리프트는 이동평균이 적응해 미탐된다"와 모순되고,
    2일차의 "사람이 못 알아챌 만큼 미묘하다"도 무너진다.
...
@router.post("/control-lock", summary="제어 API 개방/잠금 — 교안상 3일차에 개방")
