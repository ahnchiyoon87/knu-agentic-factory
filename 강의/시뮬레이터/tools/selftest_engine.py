...
for scale, virt_step in ((1, 1.0), (60, 60.0), (80, 60.0), (120, 60.0)):
    ...
    if scale == 1:
        ...
        check(f"x{scale}: 샘플당 상승폭이 7일치 CSV 와 동일",
              abs(per_sample - csv_per_sample) < 1e-6,
              f"{per_sample:.6f}℃ vs 7일치 CSV {csv_per_sample:.6f}℃")
...
# =============================================================================
print("\n11. 주입 만료 — 끝난 주입이 「끝났다」고 표시되는가")
# =============================================================================
# 물리는 is_live() 가 종료시각을 봐서 알아서 멈춘다. 문제는 표시다.
# 만료 처리를 안 하면 강사 콘솔의 「진행 중인 주입」에 끝난 것이 계속 남고,
# 「제어는 열렸는데 주입이 없습니다」 경고가 영영 안 뜬다.
# 학생이 감지할 대상이 사라진 바로 그 순간에 경고가 침묵하는 셈이다.
eng, t0 = new_engine()
f = eng.factories["S01"]
now = run(eng, t0, 10)
sp = {"duration_seconds": 30.0}
eng.injections[20] = Injection(
    id=20, tenant_id="S01", equipment_id="EQ-01", kind="sensor_dropout",
    params=sp, starts_at=now, ends_at=default_end("sensor_dropout", sp, now, PROFILE),
)
check("주입 직후에는 진행 중", eng.expire_injections(now) == []
      and eng.injections[20].active)
now = run(eng, now, 40)                      # 종료시각을 넘긴다
check("물리는 이미 멈춰 있다 (값이 돌아옴)", f.equipment["EQ-01"].temperature is not None)
done = eng.expire_injections(now)
check("끝난 주입이 만료 목록으로 나온다", done == [20], str(done))
check("만료 뒤에는 진행 중이 아니다 — 콘솔이 거짓말하지 않는다",
      not eng.injections[20].active)
check("두 번 불러도 다시 나오지 않는다 (DB 갱신이 반복되지 않음)",
      eng.expire_injections(now) == [])
