...
배정은 학생이 `uv run 내번호.py` 로 직접 받습니다 — 서버가 안 나간 번호를 하나씩 붙여 줍니다.
현황·회수는 `uv run tools/자리배정.py`.
...
| 제어 API (3일차) | `X-Access-Key` 헤더 | 네임스페이스마다 키가 다릅니다. 학생이 `uv run 내번호.py` 를 돌리면 자동으로 채워집니다 — 강사가 나눠 주지 않습니다 |
...
2일차에 강사가 결측을 주입하지 않더라도, 시드에 넣어 두면 3일차까지 같은 대시보드를 씁니다.
