...
def _다듬기(s: str) -> str:
    """붙여넣기로 딸려 오는 **보이지 않는 글자**를 털어 낸다.

    단톡방·문서에서 주소를 복사해 붙이면 BOM(U+FEFF)이나 폭 없는 공백이
    같이 붙는 일이 있다. 눈에는 안 보이는데 `주소 http://` 처럼 엉뚱한 실패로
    나와서, 학생은 자기가 뭘 잘못 눌렀는지 알 수 없다.
    """
    for c in ("﻿", "​", "‌", "‍", " "):
        s = s.replace(c, "")
...
def 주소확인(주소: str, 물어봐도되나: bool) -> str:
    ...
    print()
    try:
        다시 = _다듬기(input(f"  Enter 를 누르면 {기본서버} 로 갑니다: "))
    except EOFError:
        다시 = ""
    if not 다시:
        return 기본서버
