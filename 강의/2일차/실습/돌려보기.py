...
def main() -> int:
    ...
    # ── 이번 판 요약 ───────────────────────────────────────────────────────
    #   ★ 「통과」 도장을 찍지 않는다. 오탐만 보고 판정하면 k 를 크게 올려
    #     아무것도 안 잡는 값이 「통과」로 나온다 — 학생은 그것이 최선인 줄 알고
    #     끝낸다. 그러면 이 15분이 「숫자 크게 넣으면 이기는 게임」이 된다.
    #
    #   그래서 **잡은 개수를 먼저, 오탐을 그 뒤에** 놓는다. 순서가 곧 메시지다.
    #   둘을 한 줄에 같이 두면 한쪽만 보고 판단할 수가 없다.
    print("\n" + "=" * 74)
    print(f"이번 판   잡은 것 3종 중 {caught}종   ·   오탐 하루 {per_day:.0f}번")
    print("-" * 74)

    if caught == 0:
        print("  아무것도 못 잡고 있습니다. 오탐이 적은 것은 그 때문입니다.")
    else:
        print("  찾을 것 —  잡은 개수를 지키면서, 오탐을 가장 적게 만드는 값")
        print("             관제 담당이 실제로 확인하려면 하루 30번 언저리여야 합니다.")
        print()
        print("  ※ 오탐만 줄이는 것은 쉽습니다. k 를 크게 올리면 거의 안 울립니다 —")
        print("    아무것도 안 잡아서 그렇습니다. 위 두 숫자를 반드시 같이 보세요.")
    print()
    # ★ 예시 값에 정답(k 3.5)을 쓰지 않는다. 찾는 것이 이 실습의 전부인데
    #   화면이 매번 답을 찍어 주면 학생은 그것만 쳐 보고 끝낸다.
    print("  손잡이는 둘뿐입니다. 한 번에 하나씩. 값은 자유입니다 — 3.2 든 4.3 이든.")
    print(f"    uv run 돌려보기.py --k {args.k + 0.2:.1f}")
    print(f"    uv run 돌려보기.py --window {90 if args.window != 90 else 45}")
    print("=" * 74)
    return 0
