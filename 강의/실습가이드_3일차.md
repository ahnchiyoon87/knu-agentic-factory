...
**① `mcp_server.py` 를 열고 `Ctrl`+`F` 로 `여기부터 구현합니다` 를 칩니다**

정확히 **두 군데**가 표시됩니다. 위쪽이 `detect_anomaly`, 아래쪽이 `query_equipment` 입니다.

**② 위쪽 주석 아래에 통으로 씁니다**

지울 것도 고칠 것도 없습니다. 주석은 지우든 두든 상관없습니다.
...
**오래 붙잡지 마세요.** 그 자리만 채우고, 나머지는 내가 쓴 것 그대로 남습니다.
...
**② `Ctrl`+`F` 로 `여기부터 구현합니다` 를 쳐서 그 주석 아래에 씁니다**

이 파일에서는 한 군데만 걸립니다. 지울 것도 고칠 것도 없고, 그 아래에 통으로 씁니다.
...
1번을 빠뜨리면 **조치가 먹힌 뒤에도 계속 이상이라고 나옵니다.** 이상은 보는 구간 안의 짧은 토막일 뿐인데, 안 자르면 온도가 내려간 뒤에도 그 토막이 구간에 남아 계속 울립니다. 실측으로 최근 600개만 보면 **+1.2℃** 로 또렷하게 잡히고 이상이 끝나면 스스로 조용해지는데, 안 자르면 **+0.5℃** 로 흐려진 채 끝나고도 계속 울렸습니다.
...
**④ 아래 세 덩이를 지시문에 그대로 넣습니다** (둘째 덩이는 두 줄입니다)
