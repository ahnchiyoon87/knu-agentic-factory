...
with sync_playwright() as p:
    b = p.chromium.launch()
    pg = b.new_page(viewport={"width": 800, "height": 1150}, device_scale_factor=2)
    pg.goto((out / "미리보기.html").resolve().as_uri())
    pg.wait_for_timeout(1200)
    pg.evaluate("""() => {
        const t = [...document.querySelectorAll('h2')].find(x => x.textContent.includes('2 · 내 공장'));
        if (t) t.scrollIntoView({block:'start'});
    }""")
    pg.wait_for_timeout(400)
    pg.screenshot(path=str(out / "지면_2번.png"))
    b.close()
print("찍음")
