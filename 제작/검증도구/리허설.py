...
def 일일차_이상감지() -> None:
    ...
    check("빈 뼈대로 실행해도 무엇을 채워야 하는지 알려주고 끝난다 (죽지 않는다)",
          (not ok) and "빈칸" in out and 살아있나(out), msg)
    ...
    # 남은 것은 **어디가 왜 틀렸는지 짚어 주는 것**뿐이라, 그걸 짚는지를 본다.
    check("안 채운 자리를 「아직 안 채웠습니다」로 콕 짚어 준다 (답은 안 준다)",
          out.count("아직 안 채웠습니다") == 3 and "`...` 를 고칩니다" in out, msg)
...
def 이일차_폐루프(s: Sim, base: str, ns: str, keys: dict, quick: bool) -> None:
    ...
    #   신호가 +1.2℃ 에서 +0.5℃ 로 흐려지고, **이상이 끝난 뒤에도 계속 울린다**
    #   (실측 — 안 자르면 가상 300분 뒤까지 +0.52℃ 로 울렸다).
