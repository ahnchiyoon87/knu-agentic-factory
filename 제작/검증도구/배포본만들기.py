...
정답/ 은 **같이 나간다.**
  `점검.py --정답` 가 이걸 읽는다. 없으면 「완성본을 못 찾았습니다」로 끝나고,
...
def 검증(out: Path) -> int:
    ...
    돌린다(lab1, ["점검.py"], "안 채운 자리를 콕 짚어 준다", "`...` 줄을 고칩니다")
    ...
    try:
        ...
        돌린다(lab1, ["점검.py", "--열기", "1"], "옛 이름 `--열기` 도 그대로 받는다",
               "완성본으로 채웠습니다")
        (lab1 / "detect.py").write_bytes(detect_원본)

        for n in ("1", "2", "3"):
            돌린다(lab1, ["점검.py", "--정답", n], f"2일차 --정답 {n} (정답/ 이 같이 왔는가)",
                   "완성본으로 채웠습니다")
        돌린다(lab1, ["점검.py"], "--정답 뒤 점검이 3/3", "3개 중 3개 통과")
        돌린다(lab1, ["run.py"], "★ --정답 뒤 run.py 가 끝까지 간다", "분명하게 잡은 것")

        for n in ("1", "2"):
            돌린다(lab2, ["점검.py", "--정답", n], f"3일차 오전 --정답 {n}",
                   "완성본으로 채웠습니다")
...
def main() -> int:
    ...
    print("  정답/ 포함 (점검.py --정답 이 이걸 읽습니다)")
