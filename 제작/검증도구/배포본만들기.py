...
무엇이 들어가는가
  2일차/ · 3일차/        실습 코드와 실습가이드 (강의자료 PNG 는 뺀다 — 무겁고 강사 것)
...
특강 = REPO / "특강"                              # 특강/{일차}/{실습가이드.md·실습/} 이 학생 저장소가 된다
DATA = ROOT / "센서데이터" / "데이터"
...
| **3일차 오전** | [`3일차/실습가이드.md`](3일차/실습가이드.md) | `3일차/실습/도구만들기` |
| **3일차 오후** | 같은 가이드 §2 | `3일차/실습/폐루프` |
...
```
2일차   내 공장을 열어 본다  →  이상을 잡는 코드를 직접 짠다
                                  (detect.py 의 빈칸 3개)
                                        │
                                        │  ← 어제 짠 그 코드를 그대로 씁니다
                                        ▼
3일차   오전  내 코드를 AI 가 부를 수 있는 도구로 내놓는다
...
1. **내 번호를 적어 두세요.** 2일차 맨 처음에 화면에 크게 뜹니다 (예: `S07`).
   자리를 옮기거나 컴퓨터를 바꿔도 그 번호 하나면 원래 공장으로 돌아옵니다.
2. **`2일차/실습` 폴더를 지우지 마세요.** 3일차가 그 안의 `detect.py` 를 그대로 불러 씁니다.
...
3일차에 쓸 설정도 이때 자동으로 채워집니다 — 손으로 옮겨 적을 것이 없습니다.
...
```
2일차/실습/          detect.py 의 세 함수를 채웁니다
3일차/실습/도구만들기/  내 알고리즘을 MCP 도구로 내놓습니다
3일차/실습/폐루프/     감지·진단·조치 에이전트를 채웁니다
