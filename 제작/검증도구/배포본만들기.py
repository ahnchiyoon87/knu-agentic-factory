...
정답/ 은 **같이 나간다.**
  `확인.py --정답` 가 이걸 읽는다. 없으면 「완성본을 못 찾았습니다」로 끝나고,
  실습가이드에 적어 둔 이탈 방지 마지막 수단이 거짓말이 된다.
  3분 막혀서 이탈하는 쪽이 훔쳐보기보다 훨씬 비싸다는 판단이다.

어디에 만드나 — **풀린 채로** `제작/산출물/k-precision-lab/`

  최종본(`경남대 AI 특강 (8월)/`) 바로 옆이다. 풀린 채로 있으므로 한 줄 고치고
  **압축을 풀 것 없이 바로 눈으로** 확인된다.

  왜 최종본 **안**이 아니라 옆인가 — 최종본 폴더는 통째로 드라이브에 올라간다.
  풀린 폴더가 그 안에 있으면 학생이 zip 과 폴더 둘을 보고 어느 것을 받을지 헷갈리고,
  폴더째 받으면 파일을 하나씩 눌러 받게 된다. 그래서 올라가지 않는 자리에 둔다.

  학생에게 줄 zip 은 `드라이브폴더만들기.py` 가 이 폴더를 압축해 최종본에 넣는다 —
  풀린 것은 하나뿐이고 zip 은 그 그림자다.

사용법
    python 배포본만들기.py                      위 자리에 만든다
...
DATA = ROOT / "센서데이터" / "데이터"
# 학생 실습파일이 **풀린 채로** 사는 자리. zip 은 여기서 만들어진다.
실습원본 = REPO / "제작" / "산출물" / "k-precision-lab"
...
def 검증(out: Path) -> int:
    ...
    # `--힌트 1/2` 는 없앴다. 필요한 것은 처음부터 그 자리 주석에 다 있고,
    # 점검이 하는 일은 **어디가 왜 틀렸는지 짚는 것**뿐이다.
    # 뼈대를 빈칸(`...`)으로 바꾼 뒤로는 안내도 그 줄을 가리킨다.
    돌린다(lab1, ["확인.py"], "안 채운 자리를 콕 짚어 준다", "`...` 를 고칩니다")
    돌린다(lab2, ["확인.py"], "도구만들기 확인.py", "config.json")

    # ★ 이탈 방지 마지막 수단이 배포본에서 실제로 되는가 — 정답/ 이 빠지면 여기서 걸린다
    detect_원본 = (lab1 / "detect.py").read_bytes()
    mcp_원본 = (lab2 / "mcp_server.py").read_bytes()
    try:
        # 옛 이름도 계속 받아야 한다 — 강사 문서·기억·이전 기수 자료가 `--열기` 를 쓴다.
        # **먼저** 본다. 뒤에 하면 파일을 되돌려야 해서 3/3 검사가 깨진다.
        돌린다(lab1, ["확인.py", "--열기", "1"], "옛 이름 `--열기` 도 그대로 받는다",
               "완성본으로 채웠습니다")
        (lab1 / "detect.py").write_bytes(detect_원본)

        for n in ("1", "2", "3"):
            돌린다(lab1, ["확인.py", "--정답", n], f"2일차 --정답 {n} (정답/ 이 같이 왔는가)",
                   "완성본으로 채웠습니다")
        돌린다(lab1, ["확인.py"], "--정답 뒤 점검이 3/3", "3개 중 3개 통과")
        돌린다(lab1, ["돌려보기.py"], "★ --정답 뒤 돌려보기.py 가 끝까지 간다", "분명하게 잡은 것")

        for n in ("1", "2"):
            돌린다(lab2, ["확인.py", "--정답", n], f"3일차 오전 --정답 {n}",
                   "완성본으로 채웠습니다")
        돌린다(lab2, ["mcp_server.py", "--check"], "★ --정답 뒤 도구 2개가 실제로 돈다",
               "sample_count")
    ...
    돌린다(lab3, ["loop.py", "--정답", "1", "--check"], "3일차 오후 --정답 1 (감지만)",
           "완성본으로 채웠습니다")
    돌린다(lab3, ["loop.py", "--정답", "전부", "--check"],
           "★ --정답 전부 (세 자리 다 막힌 학생도 갈 수 있는가)", "세 자리를 다 열었습니다")
    ...
    어제.rename(치운곳)
    try:
        돌린다(lab2, ["확인.py"], "2일차 폴더가 없을 때 — 점검이 사람 말로 알려 준다",
...
def main() -> int:
    ...
    # 최종본 바로 옆에 **풀린 채로** 만든다. 전에는 산출물/배포본/ 에 한 겹 더
    # 들어가 있어서, 학생이 받을 것을 보려면 zip 을 풀어야 했다.
    # 풀린 것은 이 하나뿐이다 — 사본을 두지 않는다.
    ap.add_argument("--out", default=str(실습원본))
    ...
    #   전에는 이 되돌리기가 `--검증` 안에만 있어서, 그냥 돌리면 강사 작업본의
    #   `shared_api=127.0.0.1` · `tenant=S00` 이 박힌 채 배포본이 만들어졌다.
    #   드라이브 묶기가 그걸 잡아 멈추는데, 정작 안내는 「그냥 다시 돌리라」였다 —
    #   다시 돌려도 같은 상태가 나오므로 빠져나갈 수가 없었다.
    설정되돌리기(out)
