...
class Runner:
    def __init__(self) -> None:
        ...
        # 배속 N(>1) 이면 가상 샘플 간격을 60초(= W5 레거시 CSV 격자)로 고정하고
        # 그 샘플을 실제로는 60/N 초마다 내보낸다.
        #   x60  → 실제 1.00초마다 1분치   (교안의 "1초 주기 변동"이 그대로 유지된다)
        #   x80  → 실제 0.75초마다 1분치
        #   x120 → 실제 0.50초마다 1분치
        # 기울기를 건드리지 않으므로 샘플당 상승폭이 W5 와 같다(0.5℃/h → 0.00833℃/샘플).
