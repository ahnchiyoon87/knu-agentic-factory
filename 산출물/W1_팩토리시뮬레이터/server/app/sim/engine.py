...
이 결합이 없으면 Day 4 의 「행동 → 다시 인지」 폐루프가 닫히지 않는다.
...
class Factory:
    ...
    def tick(self, now: datetime, dt: float, injections: list[Injection],
        ...
        공장을 가로지르는 데 0.2초가 걸려 눈에 안 보인다. Day 4 클라이맥스가
    ...
    def _tick_equipment(self, now: datetime, dt: float) -> None:
        ...
        for eq in self.equipment.values():
            ...
            #    노이즈가 없으면 단순 임계값으로 전부 잡혀 Day 3 실습이 성립하지 않는다.
            if eq.sensor_online:
