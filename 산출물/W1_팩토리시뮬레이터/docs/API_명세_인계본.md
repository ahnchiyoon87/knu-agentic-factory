...
경남대 RISE 피지컬AI 사관학교 8월 특강 · W1 산출물 부속

**용도** — Lab 1-1 Step 1 프롬프트 시드를 강의자료 쪽에서 작성하기 위한 API 사실 자료입니다.
...
배정표(학생 N번 ↔ `S01`~`S39`)는 강사가 정합니다. 시뮬레이터는 누가 어느 것을 쓰는지 모릅니다.
...
| 구간 | 인증 | 이유 |
|---|---|---|
| **읽기 API (Day 1~3)** | **없음** | Day 1 Step 1 의 진입장벽을 없애기 위한 결정입니다. 네임스페이스 이름만 알면 읽힙니다 |
| 제어 API (Day 4) | `X-Access-Key` 헤더 | 네임스페이스마다 키가 다릅니다. 강사 콘솔에서 확인해 배포합니다 |
| 강사 콘솔 API | `X-Instructor-Token` 헤더 | 강사 전용 |

**Lab 1-1 시드에는 인증이 필요 없습니다.** `fetch(url)` 만으로 됩니다.
...
> **제어 API 4종은 Day 1~3 에 잠겨 있습니다.** 호출하면 `403` 과 함께
> *"제어 API 는 아직 잠겨 있습니다. 교안상 Day 4 에 개방됩니다."* 가 돌아옵니다.
> Day 1 시드에 제어를 넣지 마십시오.

---

## 4. 권장 폴링 주기 — **2초**

교안 Day 1 강사 노트가 *"시뮬레이터 API를 얼마나 자주 불러올지(폴링 주기)를 가이드에 고정해서 적어 두세요"* 라고 지시한 항목입니다.
...
Day 1 에 강사가 결측을 주입하지 않더라도, 시드에 넣어 두면 Day 3 까지 같은 대시보드를 씁니다.
...
- 온도는 rpm 의 함수입니다. Day 4 에 감속하면 온도가 실제로 내려갑니다
- `clock.time_scale` 이 1보다 크면 강사가 배속 시연 중입니다. Day 1~3 학생 실습은 항상 `1` 입니다

---

## 9. Step 2 요구사항 카드 5장 — API 대응

카드 4·5 에 대응 API 가 **없는 것은 의도된 것**입니다(교안이 Day 2 에서 스펙으로 다시 풀게 설계).
...
| `403` | 제어 API 호출 | Day 1~3 에는 잠겨 있음. 읽기만 쓸 것 |

---

**변경 이력** — API 가 바뀌면 이 문서를 갱신해 다시 드립니다.
현재 기준: `clock` · `control` 필드 포함 (Day 1 덱B 33장 시연 대응으로 추가됨).
