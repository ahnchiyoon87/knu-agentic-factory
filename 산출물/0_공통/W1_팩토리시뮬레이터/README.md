...
경남대 RISE 피지컬AI 사관학교 **8월 Agentic AI 특강** (4일 × 4시간 · 39명) 산출물 W1.
...
| 제어 API 4종, **Day 4 에 MCP 도구로 노출** | `api/control.py` · 기본 잠금, 콘솔에서 개방 |
| 강사 콘솔 이상 주입 3종 | `api/instructor.py` · 프리셋 5종 + 배속 |
| 학생별(팀/개인) 네임스페이스 분리 | 테넌트 컬럼 · 개인 39 + 팀 8 병행 |
...
Day 4 라이브 시연이 4시간짜리 드리프트를 강의 시간 안에 보여줘야 하는데,
기울기를 키우면 Day 1·Day 3 과 모순되기 때문이다(아래 설계 4번).
...
```bash
python tools/selftest_engine.py    # 엔진 48항목 (DB 불필요, 가상 시계 불변식 포함)
...
이 결합이 없으면 Day 4 의 「행동 → 다시 인지」가 닫히지 않는다.

부수 효과로 설비마다 정상 온도가 다르다(EQ-04 56℃ ~ EQ-05 75℃).
Day 1 슬라이드의 *"선을 66도로 낮추면 다른 설비들이 하루 종일 울린다"* 가
시뮬레이터 안에서 실제로 성립한다.
...
Day 1 서사("80도를 넘은 적이 없다")와 Day 3 미탐 학습의 전제라 의도한 것이다.

### 4. 라이브 시연은 기울기가 아니라 시계로 만든다

4시간짜리 드리프트를 강의 시간 안에 보여줘야 하는데, **기울기를 키우면 안 된다.**
샘플당 상승폭이 커져 Day 1 논지("사람이 못 알아챌 만큼 미묘하다")와
Day 3 학습("이동평균이 적응해 미탐된다")이 동시에 깨진다.
...
실측 회귀 결과 기울기 0.5012±0.038℃/h, **샘플당 0.008353℃ — W5 레거시 CSV(0.008333)와 동일**.
...
**AMR 주행만은 배속과 무관하게 항상 실시간이다**(Day 4 클라이맥스가 눈에 보여야 하므로).
...
db/migrations/
  ...
  003_seed_tenants.sql 개인 39 + 팀 8 + 편성
  004_grants.sql       공개 키 권한 회수 (건너뛰지 말 것)
...
server/
  ...
  app/
    ...
    api/
      read.py          읽기 API — Day 1부터
      control.py       제어 API 4종 — Day 4에 개방
...
Day 4 에서 학생이 이 4개를 **MCP 도구로 감싼다**(교안 명시). MCP 서버 자체는 Day 4 실습 범위다.

HITL — 교안 Day4 10~11장의 승인 관문은 학생 오케스트레이터(W6)에 두는 것이 교안 설계라
...
- **W5 센서 데이터셋** — `sensor_readings` 와 동일 스키마여야 한다. `sensor_readings_csv` 뷰가 교안 부록 A 의 6컬럼 그대로다
- **W6 폐루프 템플릿** — 위 제어 API 4종을 MCP 도구로 감싸는 오케스트레이터 뼈대
- **W8 요구사항 카드** — Day 1 Step 2 카드 5장. 필요한 읽기 API 는 이미 전부 열려 있다
