...
tenant_id 에 '*' 를 주면 전 테넌트에 동시 주입된다(Day 3 Lab 2-3 Step 2 처럼
강사가 한 번에 쏴 주는 상황).
"""
...
def _warnings() -> list[dict]:
    ...
    Day 4 는 제어를 개방한 상태로 진행하는데, 이때 배속이 1이면
...
async def time_scale(
    ...
    그러면 Day 3 의 "드리프트는 이동평균이 적응해 미탐된다"와 모순되고,
    Day 1 의 "사람이 못 알아챌 만큼 미묘하다"도 무너진다.

    배속은 1분 간격 가상 샘플을 그대로 둔 채 그 샘플을 더 빨리 내보낼 뿐이라
    샘플당 상승폭이 W5 레거시 CSV 와 동일하게 유지된다(0.5℃/h → 0.00833℃/샘플).
    """
    _auth(x_instructor_token)
...
@router.post("/control-lock", summary="제어 API 개방/잠금 — 교안상 Day 4 에 개방")
