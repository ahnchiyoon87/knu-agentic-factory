# -*- coding: utf-8 -*-
"""진단 중계 39명 동시 부하 테스트 — Day 3 §2 의 실제 조건을 재현한다.

    python tools/부하테스트_진단.py                 39명 · 2초 간격 (본번과 같은 조건)
    python tools/부하테스트_진단.py --n 39 --stagger 0    최악(전원 동시)
    python tools/부하테스트_진단.py --model gpt-5.4-nano  싼 모델로 먼저
...
무엇을 보는가
    ...
    ⑥ **정비 이력 인용률**             — WO 번호를 근거로 댄 비율. Day 3 하이라이트가 여기서 갈린다
"""
...
# W6 진단 응답 스키마와 같은 모양 (학생 코드가 보내는 것과 동일)
SCHEMA = {
    "type": "object",
    "properties": {
        "cause": {"type": "string"},
...
async def main() -> int:
    ...
    if 성공:
        ...
        print(f"\n⑤ 스키마 준수        {스키마}/{len(성공)}")
        print(f"⑥ WO-2026-0801 인용  {wo}/{len(성공)}   ← Day 3 하이라이트")
