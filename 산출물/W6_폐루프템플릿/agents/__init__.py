...
Day 1 에 배운 Orchestrator-Worker 패턴을 여기서 회수합니다.
오케스트레이터(loop.py)는 누가 무슨 일을 하는지 모릅니다. 이 표만 읽습니다.

    감지  →  진단  →  조치        기본 미션. 이 세 자리는 이미 꽂혀 있습니다.
    +α                            확장 미션. EXTRA 에 한 줄 추가하면 붙습니다.

────────────────────────────────────────────────────────────────
확장 미션 — 에이전트 하나를 더 붙이는 법 (세 줄이면 끝납니다)
...
# 기본 미션 — 교안 Day 4 4~7장의 세 책임
SENSE = detector
DIAGNOSE = diagnoser
ACT = actuator

# 확장 미션 — 여기에 추가합니다 (기본 미션은 비어 있는 채로 돕니다)
EXTRA: list = []
