...
왜 만들었는가
  ...
  Day 1 부터 2일차 까지 한 번도 안 끊고 돌리면서, 학생이 무언가를 하는
...
사용법
    ...
    python 리허설.py --token <토큰> --quick     누적 대기를 줄여 빠르게(결함 놓칠 수 있음)
    python 리허설.py --token <토큰> --base-url http://192.168.0.10:8000
...
ROOT = Path(__file__).resolve().parent          # 산출물/공통
OUT = ROOT.parent                                # 산출물/
W1 = ROOT / "시뮬레이터"
W5 = OUT / "3일차_알기" / "센서데이터"
LAB32 = OUT / "3일차_알기" / "이상감지"
LAB33 = OUT / "3일차_알기" / "도구만들기"
W6 = OUT / "4일차_움직이기" / "폐루프"
...
def day1(s: Sim, ns: str, quick: bool) -> None:
    ...
    check("팀 공간도 처음부터 떠 있다 (2일차 에 재시작할 일이 없다)", h["tenants"] >= 47,
          f"{h['tenants']}개")
...
def day2(s: Sim, ns: str) -> None:
    ...
    check("정비 이력이 온다 (1일차 진단의 재료)", m["count"] > 0, f"{m['count']}건")
...
def day3(s: Sim, ns: str) -> None:
    phase("1일차 — 알기 : 이상감지 알고리즘과 MCP 도구화")
...
def day4(s: Sim, base: str, token: str, teams: list[str], quick: bool) -> None:
    phase("2일차 — 움직이기 : 제어 개방과 폐루프")

    # 강사가 누르는 것 — 명령 하나 (검사 대상 서버를 그대로 넘긴다: 로컬/클라우드 공통)
    ok, msg = run_script(W1 / "tools" / "제어개방.py", ["--base", base], 120)
...
def main() -> int:
    ...
    try:
        ...
        run_script(W1 / "tools" / "제어개방.py", ["--end", "--base", args.base_url], 120)
        for t in [args.ns, *teams]:
            try:
                s.inst("POST", "/reset", params={"tenant_id": t})
            except Exception:                                      # noqa: BLE001
