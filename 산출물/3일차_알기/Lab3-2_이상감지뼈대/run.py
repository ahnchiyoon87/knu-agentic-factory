"""Lab 3-2 Step 2 — 내 구현을 7일치 데이터에 돌려 본다.
...
DATA = ROOT.parent / "W5_센서데이터셋" / "데이터"
...
def load():
    ...
    if not csv.exists():
        sys.exit(f"데이터가 없습니다: {csv}\n  W5_센서데이터셋 에서 python generate.py 를 먼저 실행하세요.")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Lab 3-2 이상감지 실행")
