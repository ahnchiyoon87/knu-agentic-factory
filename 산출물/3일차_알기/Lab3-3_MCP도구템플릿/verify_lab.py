"""Lab 3-3 템플릿 검증 — 코드 쪽 품질 게이트.

교안 Lab 3-3 이 요구하는 것이 이 템플릿 위에서 실제로 성립하는지 확인한다.
...
def main() -> int:
    ...
    try:
        ...
        check("채울 자리 2곳이 NotImplementedError 로 막혀 있다", all(blocked),
              f"{sum(blocked)}/2")
        check("Lab 3-2 의 detect() 를 그대로 가져다 쓴다 — '내가 짠 알고리즘을 AI가 쓴다'",
    ...
    if failures:
        ...
        return 1
    print("전 항목 통과 — Lab 3-3 이 이 템플릿 위에서 성립합니다.")
    return 0
