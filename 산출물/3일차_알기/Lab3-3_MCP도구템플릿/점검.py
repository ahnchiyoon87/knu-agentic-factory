...
def 검사_어제코드() -> tuple[bool, list[str]]:
    """Lab 3-2 의 detect 가 실제로 도는가 — 오늘 도구가 이걸 그대로 쓴다."""
    try:
        sys.path.insert(0, str(ROOT.parent / "Lab3-2_이상감지뼈대"))
        import detect as d
    except Exception as e:                                       # noqa: BLE001
        return False, [f"어제 만든 detect.py 를 못 불러옵니다 — {type(e).__name__}: {e}",
                       "Lab3-2 폴더가 옆에 있는지 확인하세요."]
    try:
        out = d.detect([1.0] * 70 + [99.0], window=60, k=3.0)
    except NotImplementedError:
        return False, ["어제 TODO 세 곳이 아직 비어 있습니다.",
                       "Lab3-2 폴더에서 `python 점검.py` 로 먼저 끝내세요.",
...
def main() -> int:
    ...
    어제ok, 어제msg = 검사_어제코드()
    print(f"\n{OK if 어제ok else NO} 0-2 · 어제 만든 detect.py")
    for m in 어제msg:
        print(f"       {m}")
