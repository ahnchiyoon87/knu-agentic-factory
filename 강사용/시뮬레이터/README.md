...
| 제어 API 4종, **2일차 에 MCP 도구로 노출** | `api/control.py` · 기본 잠금, 콘솔에서 개방 |
...
2일차 라이브 시연이 4시간짜리 드리프트를 강의 시간 안에 보여줘야 하는데,
기울기를 키우면 Day 1·1일차 과 모순되기 때문이다(아래 설계 4번).
...
이 결합이 없으면 2일차 의 「행동 → 다시 인지」가 닫히지 않는다.
...
**AMR 주행만은 배속과 무관하게 항상 실시간이다**(2일차 클라이맥스가 눈에 보여야 하므로).
...
2일차 에서 학생이 이 4개를 **MCP 도구로 감싼다**(교안 명시). MCP 서버 자체는 2일차 실습 범위다.

HITL — 교안 2일차 10~11장의 승인 관문은 학생 오케스트레이터(W6)에 두는 것이 교안 설계라
