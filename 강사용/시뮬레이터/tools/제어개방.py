...
그래서 사람 기억에 맡기지 않고 여기 한 곳에 모았다.
    ...
    python tools/제어개방.py --inject-all    개인 공장 전체에 주입  ← 수업 중 이걸 쓴다
    python tools/제어개방.py --status        지금 상태와 경고 확인
    python tools/제어개방.py --end           제어 잠금 · 주입 중단 · 배속 원복

접속 정보는 .env 에서 읽는다. 서버가 떠 있어야 한다.

※ 파일 이름이 day4 인 것은 교안이 제어를 2일차 에 열도록 잡았기 때문이다.
   우리 커리큘럼에서는 **1일차 오후**에 연다. 실습은 처음부터 끝까지 **개인 단위**이고
   팀(T1~T8)은 쓰지 않는다.
"""
...
def main() -> int:
    ...
    ap.add_argument("--end", action="store_true", help="1일차 상태로 되돌림")
    ...
    if args.end:
        ...
        print("1일차 상태로 되돌렸습니다 (제어 잠금 · 주입 중단 · 배속 60)")
        return 0
    ...
    print("\n확인")
    show_status(base, token)
    print("\n실습 때 — python tools/제어개방.py --inject-all")
    return 0
