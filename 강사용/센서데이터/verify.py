...
그래서 1일차 이상감지 학생이 짤 바로 그 알고리즘(이동 윈도 z-score)을 여기서 돌려
...
DATA = ROOT / "데이터"
W1_CONFIG = ROOT.parents[1] / "강사용" / "강사용/시뮬레이터" / "config"
...
def rolling_z(series: pd.Series, window: int) -> pd.Series:
    """1일차 이상감지 학생이 구현할 이동 윈도 z-score. TODO 3곳이 채워진 형태."""
...
def main() -> int:
    ...
    w1_sql = (ROOT.parents[1] / "강사용" / "강사용/시뮬레이터" / "db" / "migrations"
    ...
    # ------------------------------ 5. 이동 윈도 z-score (1일차 이상감지 학생 알고리즘)
