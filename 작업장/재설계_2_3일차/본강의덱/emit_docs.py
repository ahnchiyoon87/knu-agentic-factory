...
def emit(tag, title, scope_note, forbid_common):
    D.LOG.clear()
    importlib.import_module(f"build_{tag}")
    log = list(D.LOG)
    n = len(log)

    # ── [첨부]원고.md
    o = [f"# Day {tag[-1]} — 강의 원고 ({n}장)", "",
    ...
    (HERE / f"Day{tag[-1]}_[첨부]원고.md").write_text("\n".join(o), encoding="utf-8")

    # ── [설명란].md
    e = [f"# Day {tag[-1]} — 설명란 ({n}장)", "",
         "> **업로드할 파일 2개**",
         "> ① `00_디자인기준판\\디자인기준판.pdf`",
         f"> ② `Day{tag[-1]}_[첨부]원고.md` ← **이 덱 범위만 담긴 원고. 다른 덱 원고를 같이 올리지 마라.**",
         ">",
    ...
    e += ["▲ 복붙 끝 ▲", ""]
    (HERE / f"Day{tag[-1]}_[설명란].md").write_text("\n".join(e), encoding="utf-8")
    print(f"Day{tag[-1]}: 원고·설명란 {n}장")
...
if __name__ == "__main__":
    ...
    if which in ("day2", "both"):
        emit("day2", "Day 2",
             "- 이 덱은 **「그럼 누가 판단하나」라는 질문에서 끝난다.**\n"
             "- **그 질문에 답하지 마라.** 에이전트·MCP·제어·승인 관문은 전부 Day 3 몫이다. 한 글자도 꺼내지 마라.",
             COMMON_FORBID)
    if which in ("day3", "both"):
        emit("day3", "Day 3",
             "- 이 덱은 **「그런데 이건 전부 소프트웨어입니다」에서 끝난다.**\n"
             "- **PLC·래더 로직·현장 사고 사례는 Day 4 몫이다. 한 글자도 꺼내지 마라.**",
             COMMON_FORBID)
