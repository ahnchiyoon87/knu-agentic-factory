...
| 제어 API 4종, **2일차에 MCP 도구로 노출** | `api/control.py` · 기본 잠금, 콘솔에서 개방 |
...
기울기를 키우면 1일차 서사와 모순되기 때문이다(아래 설계 4번).
...
이 결합이 없으면 2일차의 「행동 → 다시 인지」가 닫히지 않는다.

부수 효과로 설비마다 정상 온도가 다르다(EQ-04 56℃ ~ EQ-05 75℃).
1일차 슬라이드의 *"선을 66도로 낮추면 다른 설비들이 하루 종일 울린다"* 가
시뮬레이터 안에서 실제로 성립한다.
...
1일차 서사("80도를 넘은 적이 없다")와 미탐 학습의 전제라 의도한 것이다.

### 4. 라이브 시연은 기울기가 아니라 시계로 만든다

4시간짜리 드리프트를 강의 시간 안에 보여줘야 하는데, **기울기를 키우면 안 된다.**
샘플당 상승폭이 커져 1일차 논지("사람이 못 알아챌 만큼 미묘하다")와
...
> 001~003 주석에 `W1`·`Day 4`·팀 편성 같은 **4일 과정 시절 표현**이 남아 있다.
...
2일차에 학생이 이 4개를 **MCP 도구로 감싼다**(교안 명시). MCP 서버 자체는 2일차 실습 범위다.

HITL — 교안 2일차 10~11장의 승인 관문은 학생 오케스트레이터(폐루프)에 두는 것이 교안 설계라
...
- **7일치 센서 CSV**(`백스테이지/센서데이터`) — `sensor_readings` 와 동일 스키마여야 한다. `sensor_readings_csv` 뷰가 교안 부록 A 의 6컬럼 그대로다
- **폐루프 템플릿**(`2일차/실습/폐루프`) — 위 제어 API 4종을 MCP 도구로 감싸는 오케스트레이터 뼈대
- **인쇄 카드**(`당일/인쇄물`) — 1일차 힌트 3장 · 2일차 최소경로 1장
