...
시뮬레이터의 config/sim_profile.json 과 layout.json 을 그대로 읽어 쓴다.
두 벌로 나누면 반드시 어긋나고, 어긋나면 "과거 + 현재가 한 테이블에
합쳐진다"가 성립하지 않는다.
...
실행:
    ...
    python generate.py --sim-config <경로>     시뮬레이터 config 디렉터리 지정
"""
...
def _시뮬레이터_config() -> Path:
    """시뮬레이터 config 폴더를 찾는다.

    경로를 박아 두면 폴더 하나만 옮겨도 조용히 깨진다 — 실제로 여기서
    경로가 겹쳐 있어 **검증기가 아예 안 돌았다.**
    그래서 위로 올라가며 실제로 있는 자리를 찾는다.
    """
    for base in (ROOT, *ROOT.parents[:3]):
        for cand in (base / "시뮬레이터" / "config",
                     base / "백스테이지" / "시뮬레이터" / "config"):
            if (cand / "sim_profile.json").is_file():
                return cand
    raise SystemExit(
        "시뮬레이터 config 를 못 찾았습니다. "
        "백스테이지/시뮬레이터/config/sim_profile.json 이 있는지 확인하세요."
    )


DEFAULT_SIM_CONFIG = _시뮬레이터_config()
...
def load_configs(w1_dir: Path) -> tuple[dict, dict, dict]:
    ...
    if not profile_path.exists() or not layout_path.exists():
        sys.exit(
            f"시뮬레이터 설정을 찾지 못했습니다: {w1_dir}\n"
            "  7일치 CSV 는 물리 파라미터를 시뮬레이터와 공유합니다(동일 스키마·동일 성질 보장).\n"
            "  --sim-config 로 백스테이지/시뮬레이터/config 경로를 지정하세요."
        )
...
def generate(cfg: dict, profile: dict, layout: dict, *,
    ...
    alpha = 1.0 - math.exp(-step_sec / tau)          # 1차 지연 (시뮬레이터 과 동일)
    ...
    for eq in layout["equipment"]:
        ...
        # 센서 결측 — 값만 빠지고 행과 run_state 는 남는다(시뮬레이터 실시간 스트림과 동일)
...
# =============================================================================
def main() -> None:
    ap = argparse.ArgumentParser(description="7일치 센서 데이터셋 생성")
    ...
    ap.add_argument("--sim-config", default=str(DEFAULT_SIM_CONFIG), help="시뮬레이터 config 디렉터리")
    args = ap.parse_args()

    cfg, profile, layout = load_configs(Path(args.sim_config))
    seed = args.seed if args.seed is not None else int(cfg["seed"])

    print("=" * 72)
    print("7일치 센서 데이터셋 생성")
    print(f"  시뮬레이터 설정   {args.sim_config}")
