...
def main() -> int:
    ...
    profile = json.loads((SIM_CONFIG / "sim_profile.json").read_text(encoding="utf-8"))
    ...
    # ---------------------------------------------------- 2. 시뮬레이터 실시간 스트림 정합
    print("\n2. 시뮬레이터 실시간 스트림과 동일 스키마인가")
    sim_sql = (SIM_CONFIG.parent / "db" / "migrations"
              / "002_views_functions.sql").read_text(encoding="utf-8")
    view_cols = [c.strip().strip('"') for c in
                 sim_sql.split("create or replace view sensor_readings_csv as")[1]
                 .split("from")[0].replace("select", "").split(",")]
    check("시뮬레이터 sensor_readings_csv 뷰와 컬럼 일치", view_cols == COLUMNS, ", ".join(view_cols))

    layout = json.loads((SIM_CONFIG / "layout.json").read_text(encoding="utf-8"))
    ...
    for eq in layout["equipment"]:
        ...
        if abs(actual - expected) > 1.2:
            ok_base = False
    check("설비별 정상 온도가 시뮬레이터 물리식과 일치", ok_base, " · ".join(detail))
