...
무엇을 하나
  ...
  3. 2일차 실습 폴더의 `config.json` 두 개를 **자동으로 채웁니다.**
...
ROOT = Path(__file__).resolve().parent          # 1일차/실습
...
def _내컴퓨터인가(주소: str) -> bool:
    ...
    번호는 받아지고(강사 서버가 아니라 자기 PC 라 실은 못 받지만) 2일차
...
def config채우기(번호: str, 키: str, 서버: str) -> list[str]:
    """2일차 실습 두 곳의 config.json 을 채운다. 손으로 옮겨 적지 않게."""
    바꾼곳 = []
    도구 = BASE / "2일차" / "실습" / "도구만들기" / "config.json"
    폐루프 = BASE / "2일차" / "실습" / "폐루프" / "config.json"
    ...
    if 도구.is_file():
        ...
        도구.write_text(json.dumps(c, ensure_ascii=False, indent=2) + "\n",
                        encoding="utf-8")
        바꾼곳.append("2일차/실습/도구만들기/config.json")
    ...
    if 폐루프.is_file():
        ...
        폐루프.write_text(json.dumps(c, ensure_ascii=False, indent=2) + "\n",
                          encoding="utf-8")
        바꾼곳.append("2일차/실습/폐루프/config.json")
...
def main() -> int:
    ...
    print()
    print("  ■ 1일차 — 크롬 주소창에 그대로 붙여넣으세요")
    print(f"     {서버}/view?tenant={번호}")
    if 바꾼곳:
        print()
        print("  ■ 2일차 설정은 이미 채워 뒀습니다 (손으로 적을 것 없음)")
        for f in 바꾼곳:
            print(f"     {f}")
    print()
    return 0
