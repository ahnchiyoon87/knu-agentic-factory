...
| 제어 API (2일차) | `X-Access-Key` 헤더 | 네임스페이스마다 키가 다릅니다. 학생이 `python 내번호.py` 를 돌리면 자동으로 채워집니다 — 강사가 나눠 주지 않습니다 |
...
> *"제어 통로가 아직 잠겨 있습니다. 2일차 오후에 강사가 엽니다. …"* 가 돌아옵니다.
> 1일차 시드에 제어를 넣지 마십시오.
...
1일차에 강사가 결측을 주입하지 않더라도, 시드에 넣어 두면 2일차까지 같은 대시보드를 씁니다.
