"""자리 배정 — 누가 어느 공장을 받았는지 보고, 필요하면 풀어 준다.
    ...
    uv run tools/자리배정.py                  지금 누가 뭘 받았나 (수시로 확인)
    uv run tools/자리배정.py --풀기 S07        S07 을 다시 안 쓰는 것으로
    uv run tools/자리배정.py --전부풀기         전부 초기화 (수업 전에 한 번)
    uv run tools/자리배정.py --base http://34.64.94.16:8000
...
언제 쓰나
  ...
  · 학생이 자기 번호를 잊었다 — 이 표에서 「표시」로 찾거나, 학생이
    `uv run 내번호.py` 를 다시 치면 화면에 다시 뜬다
"""
    ...
    if args.풀기 or args.전부풀기:
        ...
        if args.전부풀기:
            ...
            if 나간것 and not args.예:
                ...
                print("  어제 보던 공장이 아니게 됩니다. 3일차 아침이면 하지 마세요.")
                print("  2일차 아침이 맞다면 다시:  uv run tools/자리배정.py --전부풀기 --예")
                return 2
