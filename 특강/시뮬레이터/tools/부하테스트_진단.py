# -*- coding: utf-8 -*-
"""진단 중계 39명 동시 부하 테스트 — 3일차 오후 폐루프의 실제 조건을 재현한다.

    uv run tools/부하테스트_진단.py                 39명 · 2초 간격 (본번과 같은 조건)
    uv run tools/부하테스트_진단.py --n 39 --stagger 0    최악(전원 동시)
...
무엇을 보는가
    ...
    ⑥ **정비 이력 인용률**             — WO 번호를 근거로 댄 비율. 3일차 하이라이트가 여기서 갈린다
"""
...
async def main() -> int:
    ...
    if 성공:
        ...
        print(f"\n⑤ 스키마 준수        {스키마}/{len(성공)}")
        print(f"⑥ WO-2026-0801 인용  {wo}/{len(성공)}   ← 3일차 하이라이트")
